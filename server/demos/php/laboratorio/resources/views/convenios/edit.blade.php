...
<div class="page-header">
    <div><h1 class="page-title text-gradient">Editar Convenio</h1><p class="text-secondary">{{ $convenio->nombre }}</p></div>
...
<div class="card" style="max-width:700px;">
    ...
    <form method="POST" action="{{ route('convenios.update', $convenio) }}" style="padding:1.5rem;">
        @csrf @method('PUT')
        @if($errors->any())
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;">
            <div class="form-group col-span">
                ...
                <input type="text" name="nombre" class="form-control" value="{{ old('nombre', $convenio->nombre) }}" required>
            </div>
            <div class="form-group">
                <label>RUC</label>
                <input type="text" name="ruc" class="form-control" value="{{ old('ruc', $convenio->ruc) }}">
            </div>
            <div class="form-group">
                <label>Tipo</label>
                <select name="tipo" class="form-control">
                    @foreach(['Aseguradora','Empresa','Clínica','Municipalidad','Otro'] as $t)
                        <option value="{{ $t }}" {{ old('tipo', $convenio->tipo) === $t ? 'selected' : '' }}>{{ $t }}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group">
                <label>Descuento (%)</label>
                <input type="number" name="descuento_porcentaje" class="form-control" value="{{ old('descuento_porcentaje', $convenio->descuento_porcentaje) }}" min="0" max="100" step="0.01">
            </div>
            <div class="form-group">
                <label>Contacto</label>
                <input type="text" name="contacto_nombre" class="form-control" value="{{ old('contacto_nombre', $convenio->contacto_nombre) }}">
            </div>
            <div class="form-group">
                <label>Teléfono Contacto</label>
                <input type="text" name="contacto_telefono" class="form-control" value="{{ old('contacto_telefono', $convenio->contacto_telefono) }}">
            </div>
            <div class="form-group col-span">
                <label>Condiciones</label>
                <textarea name="condiciones" class="form-control" rows="3">{{ old('condiciones', $convenio->condiciones) }}</textarea>
            </div>
        </div>
