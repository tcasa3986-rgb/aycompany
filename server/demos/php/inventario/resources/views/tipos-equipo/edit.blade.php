<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="text-2xl font-bold text-white">Editar Tipo de Equipo</h2>
    ...
    <div class="py-8 px-4 sm:px-6 lg:px-8">
        <div class="max-w-2xl mx-auto bg-gray-800 rounded-xl shadow-lg p-8">
            <form action="{{ route('tipos-equipo.update', $tiposEquipo) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <input type="text" name="nombre" value="{{ old('nombre', $tiposEquipo->nombre) }}" required
                    ...
                    <div>
                        ...
                        <select name="estado" required
                            ...
                            <option value="Activo" {{ old('estado', $tiposEquipo->estado) == 'Activo' ? 'selected' : '' }}>Activo</option>
                            <option value="Inactivo" {{ old('estado', $tiposEquipo->estado) == 'Inactivo' ? 'selected' : '' }}>Inactivo</option>
                        </select>
                        @error('estado')
                            <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                        @enderror
