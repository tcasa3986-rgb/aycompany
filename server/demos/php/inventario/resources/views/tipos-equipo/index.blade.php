<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="text-2xl font-bold text-white">Tipos de Equipo</h2>
    ...
    <div class="py-8 px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-6 bg-gray-800 rounded-xl shadow-lg p-4">
            <div class="flex flex-col md:flex-row justify-between items-center gap-4 mb-4">
                <h2 class="text-xl font-bold text-white">Gestión de Tipos de Equipo</h2>
            ...
            <form action="{{ route('tipos-equipo.index') }}" method="GET"
                            ...
                            <tr class="hover:bg-gray-750 transition-colors">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                                    <div class="flex items-center space-x-3">
                                        {{-- Ver --}}
                                        <a href="{{ route('tipos-equipo.show', $tipo) }}"
                                        ...
                                        {{-- Activar/Desactivar --}}
                                        <form action="{{ route('tipos-equipo.toggle', ['tiposEquipo' => $tipo->id]) }}"
                                            method="POST" class="inline-block form-toggle-{{ $tipo->id }}">
                                            @csrf
                                            @method('PATCH')

                                            <button type="button"
                                                onclick="confirmToggle({{ $tipo->id }}, '{{ $tipo->nombre }}', '{{ $tipo->estado }}')"
                                                class="{{ $tipo->estado === 'Activo' ? 'text-red-400 hover:text-red-300 hover:bg-red-500/10' : 'text-green-400 hover:text-green-300 hover:bg-green-500/10' }} transition-colors p-1 rounded-full"
    ...
    @push('scripts')
        <script>
            function confirmToggle(id, name, status) {
                ...
                Swal.fire({
                    title: `¿${action.charAt(0).toUpperCase() + action.slice(1)} Tipo de Equipo?`,
                    text: `¿Estás seguro de que deseas ${action} el tipo de equipo "${name}"?`,
                    icon: type,
