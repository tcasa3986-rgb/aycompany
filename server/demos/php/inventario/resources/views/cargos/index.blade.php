<x-app-layout>
    <x-slot name="header">
        {{-- The header content has been moved to the main content area --}}
    </x-slot>
    ...
    <div class="py-8 px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-6 bg-gray-800 rounded-xl shadow-lg p-4">
            <div class="flex flex-col md:flex-row justify-between items-center gap-4 mb-4">
                <h2 class="text-xl font-bold text-white">Gestión de Cargos</h2>
                            ...
                            <tr class="hover:bg-gray-750 transition-colors">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <div class="flex items-center space-x-3">
                                        ...
                                        {{-- Activar/Desactivar --}}
                                        <form action="{{ route('cargos.toggle', $cargo) }}" method="POST"
                                            class="inline-block form-toggle-{{ $cargo->id }}">
                                            @csrf
                                            @method('PATCH')

                                            <button type="button"
                                                onclick="confirmToggle({{ $cargo->id }}, '{{ $cargo->nombre }}', '{{ $cargo->estado }}')"
                                                class="{{ $cargo->estado == 'Inactivo' ? 'text-green-400 hover:text-green-300 hover:bg-green-500/10' : 'text-red-400 hover:text-red-300 hover:bg-red-500/10' }} transition-colors p-1 rounded-full"
                                                title="{{ $cargo->estado == 'Inactivo' ? 'Activar' : 'Desactivar' }}">
        ...
        <script>
            function confirmToggle(id, name, status) {
                ...
                Swal.fire({
                    ...
                    text: `¿Estás seguro de que deseas ${action} el cargo "${name}"?`,
                    icon: type,
