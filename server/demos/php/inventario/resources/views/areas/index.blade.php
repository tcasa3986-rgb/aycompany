    ...
    <div class="py-8 px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-6 bg-gray-800 rounded-xl shadow-lg p-4">
            <div class="flex flex-col md:flex-row justify-between items-center gap-4 mb-4">
                <h2 class="text-xl font-bold text-white">Gestión de Áreas</h2>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <div class="flex items-center space-x-3">
                                        ...
                                        <form action="{{ route('areas.toggle', $area) }}" method="POST"
                                            class="inline-block form-toggle-{{ $area->id }}">
                                            @csrf
                                            @method('PATCH')

                                            <button type="button"
                                                onclick="confirmToggle({{ $area->id }}, '{{ $area->nombre }}', '{{ $area->estado }}')"
                                                class="{{ $area->estado == 'Inactivo' ? 'text-green-400 hover:text-green-300 hover:bg-green-500/10' : 'text-red-400 hover:text-red-300 hover:bg-red-500/10' }} transition-colors p-1 rounded-full"
                                                title="{{ $area->estado == 'Inactivo' ? 'Activar' : 'Desactivar' }}">
    ...
    @push('scripts')
        ...
        <script>
            function confirmToggle(id, name, status) {
                ...
                Swal.fire({
                    title: `¿${action.charAt(0).toUpperCase() + action.slice(1)} Área?`,
                    text: `¿Estás seguro de que deseas ${action} el área "${name}"?`,
                    icon: type,
