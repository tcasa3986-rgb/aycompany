<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="text-2xl font-bold text-white">Editar Rol: {{ $role->name }}</h2>
            ...
            <form action="{{ route('admin.roles.update', $role) }}" method="POST">
                ...
                <div class="mb-8">
                    ...
                    <input type="text" name="name" value="{{ old('name', $role->name) }}" required
                        ...
                        class="w-full bg-gray-700 border-gray-600 text-white rounded-lg focus:ring-2 focus:ring-blue-500 @error('name') border-red-500 @enderror {{ $role->name === 'Administrador' ? 'opacity-50 cursor-not-allowed' : '' }}"
                        placeholder="Ej: Editor, Supervisor">
                    @error('name')
                        <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                    @enderror
                    @if($role->name === 'Administrador')
                        <p class="text-gray-400 text-xs mt-1">El nombre del rol de Administrador no puede ser cambiado.</p>
                    @endif
                </div>
