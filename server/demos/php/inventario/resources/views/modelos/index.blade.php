    ...
    <div class="py-8 px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-6 bg-gray-800 rounded-xl shadow-lg p-4">
            <div class="flex flex-col md:flex-row justify-between items-center gap-4 mb-4">
                <h2 class="text-xl font-bold text-white">Gestión de Modelos</h2>
                        ...
                        @forelse($modelos as $modelo)
                            <tr class="hover:bg-gray-750 transition-colors">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="text-sm font-semibold text-white">{{ $modelo->nombre }}</span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-300">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                                    <div class="flex items-center space-x-3">
                                        ...
                                        {{-- Activar/Desactivar --}}
                                        <form action="{{ route('modelos.toggle', $modelo) }}" method="POST"
                                            class="inline-block form-toggle-{{ $modelo->id }}">
                                            @csrf
                                            @method('PATCH')

                                            <button type="button"
                                                onclick="confirmToggle({{ $modelo->id }}, '{{ $modelo->nombre }}', '{{ $modelo->estado }}')"
                                                class="{{ $modelo->estado === 'Activo' ? 'text-red-400 hover:text-red-300 hover:bg-red-500/10' : 'text-green-400 hover:text-green-300 hover:bg-green-500/10' }} transition-colors p-1 rounded-full"
                                                title="{{ $modelo->estado === 'Activo' ? 'Desactivar' : 'Activar' }}">
                                                @if($modelo->estado === 'Activo')
                                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    @push('scripts')
        <script>
            function confirmToggle(id, name, status) {
                ...
                Swal.fire({
                    title: `¿${action.charAt(0).toUpperCase() + action.slice(1)} Modelo?`,
                    text: `¿Estás seguro de que deseas ${action} el modelo "${name}"?`,
                    icon: type,
