...
<div style="margin-bottom:16px;">
    <a href="{{ route('personal.show', $personal) }}" class="btn btn-secondary"><i class="fas fa-arrow-left"></i> Volver</a>
</div>
...
<div class="card" style="margin-bottom:20px;">
    <div class="card-header"><span class="card-title"><i class="fas fa-edit" style="color:#3b82f6;margin-right:8px;"></i>Editar Personal</span></div>
    <div class="card-body">
        <div class="grid grid-2">
            <div class="form-group">
                <label class="form-label">DNI *</label>
                <input type="text" name="dni" class="form-control {{ $errors->has('dni')?'is-invalid':'' }}"
                    value="{{ old('dni', $personal->dni) }}" required>
            ...
            <div class="form-group">
                ...
                <select name="tipo" class="form-control" required>
                    ...
                    <option value="{{ $val }}" {{ old('tipo',$personal->tipo)===$val ? 'selected':'' }}>{{ $label }}</option>
                    @endforeach
                </select>
            </div>
        </div>
        ...
        <div class="grid grid-3">
            <div class="form-group">
                ...
                <input type="date" name="fecha_ingreso" class="form-control"
                    value="{{ old('fecha_ingreso', $personal->fecha_ingreso?->format('Y-m-d')) }}" required>
            </div>
            <div class="form-group">
                <label class="form-label">Salario (S/.) *</label>
                <input type="number" name="salario" class="form-control" step="0.01" min="0"
                    value="{{ old('salario', $personal->salario) }}" required>
            </div>
            <div class="form-group">
                <label class="form-label">Estado</label>
                <select name="estado" class="form-control">
                    @foreach(['activo','inactivo','licencia'] as $e)
                    <option value="{{ $e }}" {{ old('estado',$personal->estado)===$e ? 'selected':'' }}>{{ ucfirst($e) }}</option>
                    @endforeach
                </select>
            </div>
        </div>
        <div class="form-group">
            ...
            <input type="text" name="direccion" class="form-control" value="{{ old('direccion', $personal->direccion) }}">
        </div>
    </div>
</div>

<div style="display:flex;gap:12px;justify-content:flex-end;">
    <a href="{{ route('personal.show', $personal) }}" class="btn btn-secondary">Cancelar</a>
