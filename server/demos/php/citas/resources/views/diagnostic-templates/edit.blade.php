<x-app-layout>
    <x-slot name="header">Editar Plantilla de Diagnóstico</x-slot>

    <div class="max-w-4xl mx-auto">
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
            <form method="POST" action="{{ route('diagnostic-templates.update', $diagnosticTemplate) }}"
                class="space-y-6">
                @csrf @method('PUT')
                ...
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $diagnosticTemplate->name) }}" required
                ...
                <div class="mb-2 relative">
                    ...
                    <input type="text" id="icd10_search" placeholder="Escriba para buscar enfermedad o código..."
                        value="{{ old('icd_code', $diagnosticTemplate->icd_code) }}"
                    ...
                    <input type="hidden" name="icd_code" id="icd_code_input"
                        value="{{ old('icd_code', $diagnosticTemplate->icd_code) }}">
                </div>
                ...
                <div>
                    ...
                    <textarea name="diagnosis_text" rows="3" required
                        class="w-full rounded-lg border border-gray-200 px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400 @error('diagnosis_text') border-red-400 @enderror">{{ old('diagnosis_text', $diagnosticTemplate->diagnosis_text) }}</textarea>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <textarea name="treatment_text" rows="3"
                            class="w-full rounded-lg border border-gray-200 px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400">{{ old('treatment_text', $diagnosticTemplate->treatment_text) }}</textarea>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Prescripciones</label>
                        <textarea name="prescriptions_text" rows="3"
                            class="w-full rounded-lg border border-gray-200 px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400">{{ old('prescriptions_text', $diagnosticTemplate->prescriptions_text) }}</textarea>
                    </div>
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Notas Clínicas o Tips</label>
                    <textarea name="notes_text" rows="3"
                        class="w-full rounded-lg border border-gray-200 px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400">{{ old('notes_text', $diagnosticTemplate->notes_text) }}</textarea>
                </div>
