<x-app-layout>
    <x-slot name="header">Nuevo Tipo de Cita - Dr. {{ $doctor->user->name }}</x-slot>

    <div class="max-w-2xl mx-auto">
        <form action="{{ route('doctors.appointment-types.store', $doctor) }}" method="POST"
                ...
                <div>
                    ...
                    <select name="duration_minutes" required
                        ...
                        @foreach([10, 15, 20, 30, 40, 45, 60, 90, 120] as $min)
                            <option value="{{ $min }}" {{ old('duration_minutes', 30) == $min ? 'selected' : '' }}>{{ $min }}
                                minutos</option>
                        @endforeach
                    </select>
