<x-app-layout>
    <x-slot name="header">Editar Paciente: {{ $patient->user->name }}</x-slot>
            ...
            <form method="POST" action="{{ route('patients.update', $patient) }}" class="space-y-5">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    ...
                    <div>
                        ...
                        <select name="gender"
                            ...
                            <option value="male" {{ old('gender', $patient->gender) == 'male' ? 'selected' : '' }}>Masculino
                            </option>
                            <option value="female" {{ old('gender', $patient->gender) == 'female' ? 'selected' : '' }}>Femenino
                            </option>
                            <option value="other" {{ old('gender', $patient->gender) == 'other' ? 'selected' : '' }}>Otro
                            </option>
                        </select>
                    </div>
                    <div>
                ...
                <div>
                    ...
                    <textarea name="allergies" rows="2"
                        class="w-full rounded-lg border border-gray-200 px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400">{{ old('allergies', $patient->allergies) }}</textarea>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mt-4 pt-4 border-t border-gray-100">
                    ...
                    <div>
                        ...
                        <input type="text" name="policy_number" value="{{ old('policy_number', $patient->policy_number) }}" placeholder="Opcional"
                            class="w-full rounded-lg border border-gray-200 px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400">
                ...
                <div>
                    ...
                    <select name="primary_doctor_id"
                        ...
                        @foreach($doctors as $doc)
                            <option value="{{ $doc->id }}"
                                {{ old('primary_doctor_id', $patient->primary_doctor_id) == $doc->id ? 'selected' : '' }}>
                                Dr. {{ $doc->user->name }}
