<x-app-layout>
    <x-slot name="header">Nuevo Registro Clínico</x-slot>

    <div class="max-w-4xl mx-auto">
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
            <form method="POST" action="{{ route('medical-records.store') }}" class="space-y-6" enctype="multipart/form-data">
                @csrf
                    ...
                    <div>
                        ...
                        <select name="patient_id" required id="patient_select"
                            ...
                            @foreach($patients as $p)
                                <option value="{{ $p->id }}" {{ old('patient_id', optional($appointment)->patient_id) == $p->id ? 'selected' : '' }}>
                                    {{ $p->user->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="doctor_id" required
                            ...
                            @foreach($doctors as $doc)
                                <option value="{{ $doc->id }}" {{ old('doctor_id', optional($appointment)->doctor_id) == $doc->id ? 'selected' : '' }}>
                                    Dr. {{ $doc->user->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="appointment_id" id="appointment_select"
                            ...
                            @foreach($appointments as $appt)
                                <option value="{{ $appt->id }}" {{ old('appointment_id', optional($appointment)->id) == $appt->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
                        ...
                        <div>
                            ...
                            <input type="text" name="{{ $vital['field'] }}" value="{{ old($vital['field']) }}"
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <textarea name="chief_complaint" rows="2" required
                                ...
                                class="w-full rounded-lg border border-gray-200 px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400 @error('chief_complaint') border-red-400 @enderror">{{ old('chief_complaint') }}</textarea>
                        ...
                        <div>
                            ...
                            <textarea name="diagnosis" rows="3" required
                                ...
                                class="w-full rounded-lg border border-gray-200 px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400 @error('diagnosis') border-red-400 @enderror">{{ old('diagnosis') }}</textarea>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                ...
                                <textarea name="treatment" rows="3"
                                    ...
                                    class="w-full rounded-lg border border-gray-200 px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400">{{ old('treatment') }}</textarea>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Notas Clínicas</label>
                                <textarea name="notes" rows="3"
                                    placeholder="Observaciones adicionales..."
                                    class="w-full rounded-lg border border-gray-200 px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400">{{ old('notes') }}</textarea>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                ...
                                <textarea name="prescriptions" rows="3"
                                    ...
                                    class="w-full rounded-lg border border-gray-200 px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400">{{ old('prescriptions') }}</textarea>
                            </div>
                            <div>
                                ...
                                <textarea name="referred_to" rows="3"
                                    ...
                                    class="w-full rounded-lg border border-gray-200 px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400">{{ old('referred_to') }}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex justify-end gap-3 pt-2 border-t border-gray-100">
                    <a href="{{ route('medical-records.index') }}" class="px-5 py-2 text-sm text-gray-500 hover:text-gray-700">Cancelar</a>
