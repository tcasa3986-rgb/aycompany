<x-app-layout>
    <x-slot name="header">Editar Receta Médica #{{ str_pad($prescription->id, 5, '0', STR_PAD_LEFT) }}</x-slot>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                    ...
                    <div>
                        ...
                        <input type="date" name="date"
                            value="{{ old('date', \Carbon\Carbon::parse($prescription->date)->format('Y-m-d')) }}"
                            required
                            class="w-full rounded-lg border border-gray-200 px-4 py-2 text-sm focus:ring-2 focus:ring-blue-400">
                ...
                <div class="mt-5">
                    ...
                    <textarea name="notes" rows="2"
                        class="w-full rounded-lg border border-gray-200 px-4 py-2 text-sm focus:ring-2 focus:ring-blue-400">{{ old('notes', $prescription->notes) }}</textarea>
                </div>
            </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-4 gap-4 pr-8">
                                <div class="md:col-span-2">
                                    <label class="block text-xs font-medium text-gray-500 mb-1">Nombre del Medicamento <span
                                            class="text-red-500">*</span></label>
                                    <input type="text" name="items[{{ $index }}][medication_name]"
                                        value="{{ $item->medication_name }}" required
                                ...
                                <div>
                                    ...
                                    <input type="text" name="items[{{ $index }}][dosage]" value="{{ $item->dosage }}"
                                        class="w-full rounded-md border border-gray-200 px-3 py-1.5 text-sm focus:ring-1 focus:ring-blue-400">
                                </div>
                                <div>
                                    <label class="block text-xs font-medium text-gray-500 mb-1">Frecuencia</label>
                                    <input type="text" name="items[{{ $index }}][frequency]" value="{{ $item->frequency }}"
                                        class="w-full rounded-md border border-gray-200 px-3 py-1.5 text-sm focus:ring-1 focus:ring-blue-400">
                                </div>
                                <div>
                                    <label class="block text-xs font-medium text-gray-500 mb-1">Duración</label>
                                    <input type="text" name="items[{{ $index }}][duration]" value="{{ $item->duration }}"
                                        class="w-full rounded-md border border-gray-200 px-3 py-1.5 text-sm focus:ring-1 focus:ring-blue-400">
                                </div>
    ...
    {{-- Template for a NEW medication row --}}
    ...
    <script>
        let rowIndex = {{ max($prescription->items->count(), 1) * 10 }};
