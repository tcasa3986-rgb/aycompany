...
class SistemaController extends Controller
{
    ...
    private function ejecutarSQL(string $sql): void
    {
        ...
        // Separar por punto y coma seguido de salto de línea (excluyendo strings)
        $lineas    = [];
        $stmt      = '';
        $inString  = false;
        $strChar   = '';
        ...
        for ($i = 0; $i < $total; $i++) {
            ...
            if ($inString) {
                ...
                if ($c === "'" || $c === '"' || $c === '`') {
                    ...
                    $limpio = trim($stmt);
                    if (!empty($limpio) && !str_starts_with($limpio, '--')) {
                        $lineas[] = $limpio;
                    }
                    $stmt = '';
    ...
    private function obtenerTamanioDb(string $dbName): string
    {
        try {
            $result = DB::selectOne(
