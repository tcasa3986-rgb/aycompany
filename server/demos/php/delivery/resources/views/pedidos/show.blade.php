...
<div class="d-flex justify-content-between align-items-center mb-4">
    ...
    <div class="d-flex gap-2">
        ...
        <div class="dropdown">
            ...
            <ul class="dropdown-menu">
                <li><a class="dropdown-item" target="_blank" href="{{ route('pedidos.ticket', $pedido) }}"><i class="bi bi-receipt me-2"></i>Ticket 80mm</a></li>
                <li><a class="dropdown-item" target="_blank" href="{{ route('pedidos.comprobante', $pedido) }}"><i class="bi bi-file-earmark-pdf me-2"></i>Comprobante A4</a></li>
            </ul>
        </div>
        @if($pedido->cliente && $pedido->cliente->telefono)
        <button class="btn btn-success" onclick="notificarWhatsapp()" title="Avisar al cliente por WhatsApp">
    ...
    <div class="col-lg-4">
        ...
        <div class="card">
            ...
            <div class="card-body small">
                ...
                @if($pedido->fecha_entrega)<div class="mb-1"><strong>Entregado:</strong> {{ $pedido->fecha_entrega->format('d/m/Y H:i') }}</div>@endif
                @if($pedido->notas)<div class="mt-2 p-2 bg-light rounded"><small><strong>Notas:</strong> {{ $pedido->notas }}</small></div>@endif
                @if($pedido->motivo_cancelacion)<div class="mt-2 p-2 bg-danger-subtle rounded"><small><strong>Cancelación:</strong> {{ $pedido->motivo_cancelacion }}</small></div>@endif
            </div>
        </div>
    </div>
</div>
...
function cambiarEstado(estado) {
    if (!confirm('¿Cambiar estado a ' + estado.replace('_',' ') + '?')) return;
...
function notificarWhatsapp() {
    fetch('{{ route("pedidos.notificar", $pedido) }}', {headers: {'Accept':'application/json'}})
    .then(r => r.json()).then(d => {
        if (d.whatsapp) {
            window.open(d.whatsapp, '_blank');
        } else {
            alert('No se pudo construir el enlace de WhatsApp.');
        }
    });
}
</script>
