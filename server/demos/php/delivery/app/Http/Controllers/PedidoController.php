...
class PedidoController extends Controller
{
    ...
    public function cambiarEstado(Request $request, Pedido $pedido)
    {
        ...
        // Descontar stock al confirmar (solo una vez)
        if ($request->estado === 'confirmado' && $estadoAnterior === 'pendiente') {
            $pedido->load('items.producto');
            \App\Services\StockService::descontarPedido($pedido);
        }
        ...
        try { app(\App\Services\NotificacionService::class)->cambioEstadoPedido($pedido); } catch (\Throwable $e) {}

        return response()->json(['success' => true, 'estado' => $pedido->estado_texto]);
    }

    public function notificarWhatsapp(Pedido $pedido)
    {
        $datos = app(\App\Services\NotificacionService::class)->cambioEstadoPedido($pedido);
